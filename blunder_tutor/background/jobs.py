...
async def sync_games_job(data_dir: Path, event_bus=None) -> None:
    db_path = data_dir / "main.sqlite3"

    job_repo = JobRepository(data_dir=data_dir, db_path=db_path, event_bus=event_bus)
    ...
    for source, username in usernames.items():
        # FIX: Use the job_id returned from create_job
...
async def _sync_single_source(
    ...
    # Initialize progress
    job_repo.update_job_progress(job_id, 0, max_games)

    # Define progress callback
    def update_progress(current: int, total: int) -> None:
        job_repo.update_job_progress(job_id, current, total)
    ...
    try:
        ...
        job_repo.update_job_progress(job_id, total_processed, total_processed)
        ...
        job_repo.complete_job(job_id, {"stored": stored, "skipped": skipped})
        ...
        if auto_analyze == "true" and stored > 0:
            ...
            # FIX: Use the job_id returned from create_job
            analyze_job_id = job_repo.create_job(
                job_type="analyze",
                username=username,
                source=source,
                max_games=stored,
...
async def _analyze_new_games(
    ...
    job_repo = JobRepository(
        data_dir=data_dir, db_path=main_db_path, event_bus=event_bus
    )
    ...
    if not unanalyzed_games:
        job_repo.complete_job(job_id, {"analyzed": 0, "skipped": 0})
        return

    job_repo.update_job_progress(job_id, 0, len(unanalyzed_games))
    ...
    try:
        for i, game_id in enumerate(unanalyzed_games):
            ...
            analyzed += 1
            job_repo.update_job_progress(job_id, i + 1, len(unanalyzed_games))

        job_repo.complete_job(job_id, {"analyzed": analyzed, "skipped": 0})
...
async def analyze_games_job(
    ...
    job_repo.update_job_status(job_id, "running")
    job_repo.update_job_progress(job_id, 0, len(game_ids))
    ...
    try:
        for i, game_id in enumerate(game_ids):
            ...
            if analysis_repo.analysis_exists(game_id):
                skipped += 1
                job_repo.update_job_progress(job_id, i + 1, len(game_ids))
                continue
        ...
        job_repo.complete_job(job_id, {"analyzed": analyzed, "skipped": skipped})
