...
- **Instant move feedback**: Trainer moves now submit automatically, without a separate Submit action.
...
- **Stockfish 19**: Docker images and release CI now build with Stockfish 19.
- **Platform refresh**: Upgraded Python to 3.14 and refreshed backend and frontend dependencies.
- **Trainer actions**: Consolidated trainer move actions into a single panel.
...
- **Dark mode**: Settings → Theme now has System / Light / Dark. Dark is a full warm palette rather than an invert, and the chessboard dims with the UI unless you've set custom board colors. Custom theme colors still apply to light mode only.
