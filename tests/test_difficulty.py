...
from blunder_tutor.analysis.pipeline.steps.move_quality import compute_difficulty
...
class TestComputeDifficulty:
    def test_good_moves_have_zero_difficulty(self):
        board = _board()
        assert compute_difficulty(board, "e2e4", 0, class_to_int("good")) == 0
    ...
    def test_quiet_best_move_scores_high(self):
        ...
        score = compute_difficulty(board, "d7d6", 250, class_to_int("blunder"))
        assert score >= 40  # quiet move bonus

    def test_capture_best_move_scores_lower(self):
        # Position where best move is a capture
        board = _board("r1bqkbnr/pppppppp/2n5/4N3/8/8/PPPPPPPP/RNBQKB1R b KQkq - 0 1")
        score = compute_difficulty(board, "c6e5", 250, class_to_int("blunder"))
    ...
    def test_check_best_move_scores_lowest(self):
        ...
        score = compute_difficulty(board, "f3f7", 300, class_to_int("blunder"))
    ...
    def test_few_legal_moves_increases_difficulty(self):
        ...
        score = compute_difficulty(board, "a1b2", 250, class_to_int("blunder"))
        # capture (15) + few moves (30) = 45
        assert score >= 40

    def test_many_legal_moves_lower_difficulty(self):
        board = _board()
        score = compute_difficulty(board, "e2e4", 250, class_to_int("blunder"))
    ...
    def test_no_best_move_returns_default(self):
        board = _board()
        assert compute_difficulty(board, None, 250, class_to_int("blunder")) == 50

    def test_invalid_uci_returns_default(self):
        board = _board()
        assert compute_difficulty(board, "zzzz", 250, class_to_int("blunder")) == 50

    def test_capped_at_100(self):
        # Few legal moves + quiet move + high cp_loss = all bonuses stacking
        board = _board("8/8/8/8/8/6k1/8/5K2 w - - 0 1")
        score = compute_difficulty(board, "f1e1", 500, class_to_int("blunder"))
        assert score <= 100

    def test_large_cp_loss_quiet_move_bonus(self):
        board = _board("r1bqkbnr/pppppppp/2n5/4N3/8/8/PPPPPPPP/RNBQKB1R b KQkq - 0 1")
        low_loss = compute_difficulty(board, "d7d6", 200, class_to_int("blunder"))
        high_loss = compute_difficulty(board, "d7d6", 500, class_to_int("blunder"))
