...
describe('useDashboardFilters', () => {
  ...
  test('setDatePreset updates date range', () => {
    const { result } = renderHook(() => useDashboardFilters());
  ...
  test('setCustomDateRange clears preset', () => {
    const { result } = renderHook(() => useDashboardFilters());
    act(() => { result.current.setCustomDateRange('2024-01-01', '2024-06-01'); });
  ...
  test('setGameTypes updates and persists', () => {
    const { result } = renderHook(() => useDashboardFilters());
    act(() => { result.current.setGameTypes(['blitz', 'rapid']); });
    expect(result.current.gameTypes).toEqual(['blitz', 'rapid']);
    expect(JSON.parse(localStorage.getItem('dashboard-game-type-filters')!)).toEqual(['blitz', 'rapid']);
  });
  ...
  test('getParams includes game_types when subset selected', () => {
    const { result } = renderHook(() => useDashboardFilters());
    act(() => { result.current.setGameTypes(['blitz']); });
    const params = result.current.getParams();
    expect(params.game_types).toEqual(['blitz']);
  });

  test('getParams includes date range from preset', () => {
    const { result } = renderHook(() => useDashboardFilters());
