...
import { safeNext } from '../../src/shared/safe-redirect';

describe('safeNext', () => {
  it('returns / for null', () => { expect(safeNext(null)).toBe('/'); });
  it('returns / for empty string', () => { expect(safeNext('')).toBe('/'); });
  ...
  it('rejects backslash-prefixed variants', () => { expect(safeNext('/\\evil.com')).toBe('/'); });
