...
function getAttackers(gameObj: ChessInstance, square: string, byColor: 'w' | 'b'): Attacker[] {
    ...
    for (let r = 1; r <= 8; r++) {
      ...
      if (piece && piece.color === byColor) {
        ...
        for (const move of moves) {
          if (move.to === square) {
            attackers.push({ square: sq, piece });
            break;
          }
        }
      }
...
function getDefenders(gameObj: ChessInstance, square: string, piece: ChessPiece): Attacker[] {
  const testGame = new Chess(gameObj.fen());
...
function findHangingPieces(gameObj: ChessInstance): string[] {
...
function findKingInCheck(gameObj: ChessInstance): string | null {
  if (gameObj.in_check()) {
...
function findCheckableKing(gameObj: ChessInstance): string | null {
...
export function buildThreatHighlights(game: ChessInstance | null, showThreats: boolean): HighlightMap {
