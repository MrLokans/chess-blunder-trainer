const ACTIVE_DROPDOWN: { current: HTMLElement | null } = { current: null };

export interface DropdownHandle {
  refresh: () => void;
}

export function createDropdown(selectEl: HTMLSelectElement): DropdownHandle {
  ...
  function selectOption(li: HTMLLIElement, value: string): void {
...
export function initDropdowns(root: Document | HTMLElement = document): void {
  root.querySelectorAll<HTMLSelectElement>('select:not([data-dropdown-init])').forEach(sel => {
