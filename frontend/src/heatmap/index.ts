...
function heatmapFormatDate(date: Date): string {
...
export function renderHeatmap(containerId: string, data: HeatmapData): void {
  ...
  const todayStr = heatmapFormatDate(today);
  ...
  while (currentDate <= endDate) {
    const dateStr = heatmapFormatDate(currentDate);
...
export async function loadHeatmap(containerId: string): Promise<void> {
  try {
    const data = await client.stats.activityHeatmap() as HeatmapData;
