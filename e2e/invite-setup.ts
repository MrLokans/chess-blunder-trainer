 ...
 * lifespan startup and writes it to `<tmpDir>/invite.txt` for tests.
 *
 * Why not pre-seed the DB ourselves: Playwright starts `webServer`
 * before `globalSetup` runs. If we wipe the tmp dir in globalSetup
 ...
 */
export function captureInvite(tmpDirName: string): void {
  const projectRoot = resolve(__dirname, '..');
  const tmpDir = resolve(__dirname, tmpDirName);
